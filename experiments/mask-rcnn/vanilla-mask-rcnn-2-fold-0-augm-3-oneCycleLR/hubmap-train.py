...
# # get one example of dataset_train

# dataset_train[0][1]['boxes'].shape, dataset_train[0][1]['masks'].shape, dataset_train[0][1]['labels'].shape
...
for i, (train_index, test_index) in enumerate(kf.split(range(n_imgs))):
    ...
    train_dl = torch.utils.data.DataLoader(
        dataset_train, batch_size=3, shuffle=True, num_workers=os.cpu_count(), pin_memory=True, drop_last=True, collate_fn=utils.collate_fn)
    ...
    for epoch in range(EPOCHS):
        train_one_epoch(model, optimizer, train_dl, device, epoch, print_freq=50, copypaste=copypaste, augm=augm)
