...
class HubmapDataset(Dataset):
    ...
    def __init__(self, image_dir, labels_file, train_pct, mode='train'):
        ...
        # take meta data for the labelled images
        df_meta_labelled = df_meta[df_meta['id'].isin(ids)]
        
        # take 'source_wsi' = 1 or 2
        df_meta_labelled = df_meta_labelled[df_meta_labelled['source_wsi'].isin([1, 2])]

        n_train = int(train_pct * len(df_meta_labelled))
        
        json_labels_cleaned = []
        for label in self.json_labels:
            if label['id'] in df_meta_labelled['id'].values:
                json_labels_cleaned.append(label)
            
        self.json_labels = json_labels_cleaned
        # ----------------------------------------------------------------
