...
class HubmapDataset(Dataset):
    ...
    def __init__(self, image_dir, labels_file, n_train, mode='train'):
        ...
        if mode == 'train':
            initial_augm = [
                ...
                A.RandomSizedCrop(min_max_height=(int(512 * 0.7), int(512 * 0.9)),
    ...
    def __getitem__(self, idx):
        ...
        # compute mask distance transform
        mask = (mask.numpy() > 0).astype(np.uint8)

        # Compute the distance transform
        # num_outputs, labels, stats, centroids = cv2.connectedComponentsWithStats(mask)
        # label_masks = [labels == i for i in range(num_outputs)]
        # dist_final = np.zeros_like(mask.astype(np.float32))
        # for m in label_masks:
        #     dist = distance_transform_edt(m).astype(np.float32)
        #     max = np.max(dist)
        #     min = np.min(dist)
        #     dist = (dist - min) / (max-min) if max != min else np.zeros_like(dist)
        #     dist_final += dist
        # dist = torch.tensor(dist_final, dtype=torch.float32)
        ...
        boundaries = find_boundaries(mask, connectivity=1, mode='inner')
        ...
        return image, mask, dist, boundaries
...
indx = 157
image, mask, dist, boundaries = train_dataset[indx]
...
axs[2, 0].imshow(dist)
print("dist", dist.max())
axs[2, 0].set_title('Distance Field')

# Plot the boundaries
axs[2, 1].imshow(boundaries)
print("boundaries", boundaries.max())
axs[2, 1].set_title('Boundaries')

plt.tight_layout()
plt.show()

print(boundaries.sum() / mask.sum())
...
def criterion(pred, true):
    ...
    mask_loss = F.binary_cross_entropy_with_logits(pred['mask'], true['mask'])
    boundaries_loss = F.binary_cross_entropy_with_logits(pred['boundaries'], true['boundaries'])
    dist_loss = 10*F.mse_loss(pred['dist'], true['dist'])
    loss = mask_loss + boundaries_loss + dist_loss
    out = {
        'mask_loss': mask_loss, 
        'boundaries_loss': boundaries_loss,
        'dist_loss': dist_loss,
        'loss': loss,
    }
...
for epoch in range(1, CFG.EPOCHS+1):
    ...
    for i, (images, masks, dist, boundaries) in pbar_train:
        images, masks, dist, boundaries = images.to(CFG.device), masks.to(CFG.device), dist.to(CFG.device), boundaries.to(CFG.device)
        
        
        optimizer.zero_grad()
        pred = model(images)
        pred_masks = pred[:, 0, :, :].squeeze()
        pred_dist = pred[:, 1, :, :].squeeze()
        pred_boundaries = pred[:, 2, :, :].squeeze()
        pred = {
            'mask': pred_masks,
            'dist': pred_dist,
            'boundaries': pred_boundaries
        }
        true = {
            'mask': masks.squeeze(),
            'dist': dist.squeeze(),
        ...
        out = criterion(pred, true)
        mask_loss = out['mask_loss']
        boundaries_loss = out['boundaries_loss']
        ...
        mask_train += mask_loss.item()
        boundary_train += boundaries_loss.item()
        dist_train += dist_loss.item()
        optimizer.step()
    ...
    out_df = {}
    out_df['epoch'] = epoch
    out_df['loss_train'] = mloss_train / len(dataloader_train)
    out_df['mask_loss_train'] = mask_train / len(dataloader_train)
    out_df['boundaries_loss_train'] = boundary_train / len(dataloader_train)
    out_df['dist_loss_train'] = dist_train / len(dataloader_train)
    ...
    for i, (images, masks, dist, boundaries) in pbar_val:
        images, masks, dist, boundaries = images.to(CFG.device), masks.to(CFG.device), dist.to(CFG.device), boundaries.to(CFG.device)
        ...
        with torch.no_grad():
            ...
            boundary_val += boundaries_loss.item()
            dist_val += dist_loss.item()
    ...
    out_df['loss_val'] = mloss_val / len(dataloader_valid)
    out_df['th_best'] = best_th 
    out_df['best_fbeta'] = best_fbeta 
    out_df['mask_loss_val'] = mask_val / len(dataloader_valid)
    out_df['boundaries_loss_val'] = boundary_val / len(dataloader_valid)
    out_df['dist_loss_val'] = dist_val / len(dataloader_valid)
    
    
    # save out to csv. Rewrite every epoch
    if epoch == 1:
        df = pd.DataFrame(out_df, index=[0])
